...
export default function AvatarDropdown({account}) {
	...
	useEffect(() => {
		const result = address === '0xb6C4782d71141f980034F1ef52910f1B32216f73'
		setIsApprove(result);
	}, [address])
			...
			<Popover className="relative">
				{({open}) => (
					<>
						...
						>
							<Avatar
								imgUrl={account ? account?.avatar : avatarImgs[9]}
								...
								<div className="overflow-hidden rounded-3xl shadow-lg ring-1 ring-black ring-opacity-5">
									<div className="relative grid grid-cols-1 gap-6 bg-white dark:bg-neutral-800 py-7 px-6">
										<div className="flex items-center space-x-3">
											<Avatar imgUrl={account ? account?.avatar : avatarImgs[9]} sizeClass="w-12 h-12"/>
											<div className="flex-grow">
												<h4 className="font-semibold">{account ? account?.fullName : "Unnamed"}</h4>
												<p
													className="text-xs text-ellipsis mt-0.5">{address ? shortenMiddleString(address, 14, 4) : "--"}</p>
											</div>
										</div>
